...
module.exports = Vue.extend({
  ...
  template: `<div class="panel panel-default app-item"><div class="panel-heading"><div class="pull-right"><small class="text-success" v-if="app.info.plist">v{{app.info.plist.CFBundleShortVersionString}}</small><a class="app-action label label-danger" v-if="!app.del&amp;&amp;app.info.rules" v-on:click="appremove(app)"><i class="fa fa-trash"></i> 删除</a><span v-if="app.del" class="label label-success">已删除</span></div><span class="app-sub-icon"><i class="fa fa-microchip"></i> 子应用 :</span><strcolor v-bind:search="status.search" v-bind:text="app.name"></strcolor><small v-if="app.info.size" class="text-success margin-left-5" style="margin-right:5px;">{{app.info.size?app.info.size:'正在计算...'}}</small></div><div class="panel-body"><div class="media"><div class="media-action pull-right"><button v-if="!app.info.size" type="button" class="btn btn-success btn-xs" v-on:click="appinfo(app)" v-bind:disabled="app.loading"><i v-if="app.loading" class="fa fa-spinner fa-pulse"></i> 应用信息</button><button v-if="app.info.size" type="button" class="btn btn-info btn-xs" v-on:click="apprules(app)" v-bind:disabled="app.info.loading"><i v-if="app.info.loading" class="fa fa-spinner fa-pulse"></i>{{app.info.rules?'更新文件':'分析文件'}}</button></div><div class="media-left"><img v-if="app.info.icon" :src="app.info.icon" class="media-object"><div v-if="!app.info.icon" class="media-object"><i class="fa fa-cube"></i></div></div><div class="media-body"><div class="app-path">路径:<strcolor class="text-info" v-bind:search="status.search" v-bind:text="app.filepath"></strcolor></div><div class="app-sign" v-if="app.info.plist">应用名:<span class="text-info">{{app.info.plist.CFBundleName}}</span>标识:<span class="text-info">{{app.info.plist.CFBundleIdentifier}}</span></div><div class="app-time"><span>访问时间:<timestr v-bind:time="app.atimeMs"></timestr></span><span>修改时间:<timestr v-bind:time="app.mtimeMs"></timestr></span><span>创建时间:<timestr v-bind:time="app.birthtimeMs"></timestr></span></div></div><!-- 延迟显示定位点 --><lazy-component v-if="app.name" @show="lazyshow(app)"></lazy-component></div><div class="panel-group app-subapp" v-if="app.subapps.length"><section v-for="(subapp) in app.subapps" :key="subapp.uuid"><app-item v-bind:status="status" v-bind:app="subapp"></app-item></section></div><app-rules v-if="app.info.rules&amp;&amp;app.info.rules.length" v-bind:rules="app.info.rules"></app-rules></div></div>` //必须是单引号 模板替换标识可在配置内更改
});